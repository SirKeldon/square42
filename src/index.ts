// RXJS IMPORTS
import { animationFrameScheduler, BehaviorSubject, interval, Observable, 
    withLatestFrom, startWith, switchMap, takeWhile, repeat, 
    map, of, tap, share,  scan, skip } from 'rxjs';
...
function createGame(fps$: Observable<number>): Observable<GameOfLife> {
    ...
    return fps$.pipe(
        withLatestFrom(canCross$, running$, generation$, matrix$, survivors$, elapsed$, 
                        (_, canCross, running, generation, matrix, survivors, elapsed) => ({ 
                            canCross, running, generation, matrix, survivors, elapsed 
                        })),
        startWith({
            matrix,
...
const startGame = () => {
    newGame$ = game$.subscribe({
        next: ({ scene: { matrix, survivors, canCross, generation, elapsed } }) => render(canvas, canvasElement, { matrix, survivors, canCross, generation, elapsed }),          
    });
}
