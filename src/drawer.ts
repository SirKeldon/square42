...
const buildCanvasBackground = (tileSize: number): string => {
    ...
    <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg"> 
        <defs>
            ...
            <pattern id="grid" width="${tileSize*settings.rept}" height="${tileSize*settings.rept}" patternUnits="userSpaceOnUse">
                <rect width="${tileSize*tileSize}" height="${tileSize*tileSize}" fill="url(#smallGrid)" />
                <path d="M ${tileSize*tileSize} 0 L 0 0 0 ${tileSize*tileSize}" fill="none" stroke="gray" stroke-width="1" />
            </pattern>
        </defs>
...
const drawDot = (canvas: CanvasRenderingContext2D, 
                coord: Coordinate, 
                tileSize: number, 
                color: string) => {
    canvas.fillStyle = color;
    canvas.fillRect(calculateDrawDeviation(coord.x, tileSize), calculateDrawDeviation(coord.y, tileSize), tileSize-1, tileSize-1);
}
...
const renderGrid = (canvas: CanvasRenderingContext2D, grid: Array<Array<number>>, generation: number) => {
    ...
    grid.forEach((row, x) => {
        row.forEach((element, y) => {
            let percentile;
            if (element >= 1) {
                percentile = (element*360/generation) > 360 
                                ? (element*360/generation) / 360
                                : (element*360/generation);
                //percentile = (element/generation)*100
                //console.log(percentile);
            }

            if (element === 1)
                drawDot(canvas, {x, y}, settings.size, percentageToColor(percentile))
...
const renderSurvivors = (matrix: Array<number>) => {
    ...
    document.getElementById('summaryGenerations').innerHTML = "";
    document.getElementById('summaryAmounts').innerHTML = "";
    
    const rowGenerations = document.createElement('tr');
    const rowAmount = document.createElement('tr');
    
    matrix.forEach((survivors, index) => {
        if (survivors >= 1) {
            const rowDataGen = document.createElement('th');
            const rowDataAmount = document.createElement('td');
            rowGenerations.appendChild(rowDataGen);
            rowAmount.appendChild(rowDataAmount);
            rowDataGen.appendChild(document.createTextNode(getOrdinal(index)));
            rowDataAmount.appendChild(document.createTextNode(survivors.toString()));
        }
    })

    document.getElementById('summaryGenerations').appendChild(rowGenerations);
    document.getElementById('summaryAmounts').appendChild(rowAmount);
